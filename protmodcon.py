...
def perform_enrichment_analysis(
    ...
    hierarchy = {'ptm_name': 0, 'AA': 1, 'sec': 2, 'domain': 3, 'protein_id': 4}
    if hierarchy[x_types] > hierarchy[y_types]:
        raise ValueError("x_types must be a lower conformational level than y_types.")
    if (y_types=='sec' and len(sec_ids) != 0) \
    or (y_types=='domain' and len(sec_ids + domain_ids) != 0) \
    or (y_types=='protein_id' and len(sec_ids + domain_ids + protein_ids) != 0):
        raise ValueError("y_types must be a lower conformational level than the specified id(s).")
